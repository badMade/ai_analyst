...
Be thorough but efficient. Present results in a structured, easy-to-understand format."""
    ...
    def _execute_tool(self, tool_name: str, tool_input: dict) -> str:
        ...
        try:
            ...
            logger.warning(f"File not found error in tool {tool_name}: {e}")
            from pathlib import Path
            filename = Path(str(e).split(": ")[-1]).name if ": " in str(e) else str(e)
            return json.dumps({"error": f"File not found: {filename}"})

        except ValueError as e:
            if "outside of allowed base directory" in str(e) or "Windows-style path" in str(e):
                logger.warning(f"Security error in tool {tool_name}: Path traversal attempt or invalid path.")
                return json.dumps({"error": "Security Error"})
            logger.exception(f"Tool execution error (ValueError): {tool_name}")
