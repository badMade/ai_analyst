...
def interactive(file_path: str | None, model: str) -> None:
...
def inspect(file_path: str) -> None:
    ...
    for col, dtype, non_null, pct in zip(df.columns, df.dtypes, non_null_counts, null_pcts, strict=True):
        table.add_row(str(col), str(dtype), str(non_null), f"{pct:.1f}%")
